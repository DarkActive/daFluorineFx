...
namespace FluorineFx.IO
{
	...
	public class AMFReader : BinaryReader
	{
        ...
        // LibOfLegends fix for LoginPacket, to know if its LoginPacket
        bool _loginPacket = false;
        ...
        /// </summary>
        /// <returns>The Object deserialized from the AMF stream.</returns>
        public object ReadSpecialData(string str)
        {
            if (str == "double")
            {
                byte typeCode = this.ReadByte();
                byte[] b = new byte[] { typeCode };
                return AmfTypeTable[3][5].ReadData(this);
            }
            else if (str == "json")
            {
                byte typeCode = this.ReadByte();
                this.ReadByte();
                return ReadJson(typeCode);
            }
            else
                return AmfTypeTable[3][6].ReadData(this);
        }

        /// <summary>
        /// Reads an JSON string type
        /// </summary>
        /// <returns>The String value.</returns>
        public string ReadJson(byte type)
        {
            byte[] hex;
            string hexStr;
            string result = "";
            ...
            {
                hex = ReadBytes(1);
                hexStr = BitConverter.ToString(hex);

                if (fromHexToString(hexStr) == "{")
                {
                    startJson = true;
                }
            }
        ...
        {
            if (index >= _stringReferences.Count && _loginPacket)
                return _stringReferences[_stringReferences.Count - 1] as string;
        ...
        {
            if (index > _classDefinitions.Count && _loginPacket)
                return _classDefinitions[_classDefinitions.Count - 1] as ClassDefinition;
        ...
        internal object ReadAMF3Object(ClassDefinition classDefinition)
        {
            ...
            {
                // Fix for LibOfLegends so it nows its LoginDataPacket
                if (classDefinition.ClassName == "com.riotgames.platform.clientfacade.domain.LoginDataPacket")
                {
                    _loginPacket = true;
                }
                for (int i = 0; i < classDefinition.MemberCount; i++)
                {
                    ...
                    object value;
                    // HOT FIX for LibOfLegends not working items
                    if (key == "broadcastNotification" && _loginPacket)
                        value = ReadSpecialData("json");
                    else if (key == "clientSystemStates" && _loginPacket)
                        value = ReadSpecialData("json");
                    else
                        value = ReadAMF3Data();
                    SetMember(instance, key, value);
